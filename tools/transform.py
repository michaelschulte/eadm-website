...
def vendor_all_pdfs(attachment_files, uploads_root, images_out, files_out):
    """Copy every PDF attachment into files/, whether or not any kept
    content references it. The migration's decision was to vendor all of
    them; rewriting alone only copies the ones that happen to be linked.
    Returns (copied, unresolved) counts.
    """
    copied = unresolved = 0
...
def main():
    ...
    pdfs_copied, pdfs_unresolved = vendor_all_pdfs(
        attachment_files, UPLOADS_ROOT, IMAGES_OUT, FILES_OUT
    )

    write_content_audit(dropped, written)
    print(f"Wrote {len(written)} .qmd files, dropped {len(dropped)} items.")
    print(
        f"Vendored {pdfs_copied} PDF attachments into files/ "
        f"({pdfs_unresolved} unresolved -- not present in the export on disk)."
    )
...
def write_content_audit(dropped, written):
    ...
    for item, section, filename in sorted(
        written, key=lambda w: f"{w[1]}/{w[2]}" if w[1] else w[2]
    ):
        path = f"{section}/{filename}" if section else filename
        lines.append(f"- **{item['title']}** (WP id {item['id']}) → {path}.qmd")
    lines.append("")
